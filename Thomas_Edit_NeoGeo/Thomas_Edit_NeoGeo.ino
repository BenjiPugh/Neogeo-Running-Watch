...
long buttonHoldDelay1 = 1000;      // how long to hold the button down for mode change
long buttonHoldDelay2 = 2500;      // how long to hold down the button for toggling (start/stop, set location)
bool toggle = false;
...
void buttonCheck() {
  ...
  if (buttonState == HIGH && lastButtonState == LOW) {
    if ((millis() - buttonHoldTime) > buttonHoldDelay1 && (millis() - buttonHoldTime) < buttonHoldDelay2) {
    ...
    //Button toggling
    if ((millis() - buttonHoldTime) > buttonHoldDelay2){
     if (toggle == false) {
      toggle = true;
      buttonHoldTime = millis();
...
void compassDirection(int compassHeading) 
{
  ...
  return (deg * M_PI / 180);
        ...
        //turns latitude difference into km
        float distanceEarth() {
          float lat1r, lon1r, lat2r, lon2r, u, v;
          lat1r = deg2rad(distLat1);
          lon1r = deg2rad(distLon1);
          lat2r = deg2rad(distLat2);
          lon2r = deg2rad(distLon2);
          u = sin((lat2r - lat1r)/2);
          v = sin((lon2r - lon1r)/2);
...
//find the distance between two points, add to collective distance scale
void distMode()  {
  
  int StripSize = 16;
  uint32_t green = strip.Color(0, 100, 0); //ThomasB - If you add a 4th number to this, you can adjust the white level of the color as well
  uint32_t red = strip.Color(100, 0, 0); //ThomasB - I think setting color names to the strip color is cleaner and more informative in the rest of the code. Also means you can't fat-finger a number somewhere and have to figure out where you screwed up in the code. If you fat-finger the variable name, you just won't compile!
  uint32_t off = strip.Color(0, 0, 0);
  if (start == 1) {
    if (toggle == true) {
    int sixteenthKm = topLED;
    int KmPixel = topLED;
    float distHelper = 0;

    distDiff = 0;
    //get two seperate points
    distLat1 = fLat;
    distLon1 = fLon;
    delay(100);
          ...
          distDiff = distanceEarth();
    
           //add to total dist
           totalDist = totalDist + distDiff;
           distHelper = distHelper + distDiff;
           Serial.print(totalDist);
               
               
      //calculate the digits that will be represented by pixels           
    if (distHelper > 1) 
    {
      distHelper = distHelper - 1;
      KmPixel = KmPixel + 1;
    ...
    if (distHelper < 1)
    {
      sixteenthKm = distHelper * 16;
      
      if (sixteenthKm > 16)
        {
          sixteenthKm = sixteenthKm - 16;
        }
    } 
        
        //ThomasB - You can declare i in your for statement, rather than before. The reason you would declare it before would be if you wanted to access the value of i outside of the for loop. However, since you don't do that here, it's better memory management to declare it in the declaration of the loop so that it is released once the loop ends. Also, i++ is great shorthand for i += 1 which is shorthand for i = i + 1. For future reference, you can also do stuff like i--, i-=1 i*=1, and i/=1.
        for (int i = 0; i <= sixteenthKm; i++) {
          if (i != KmPixel){
            strip.setPixelColor(i, green);
          }
        }
        if (KmPixel != topLED){
          strip.setPixelColor(KmPixel, strip.Color(150,0,0)); //ThomasB - What is the topLED doing in here? That's just going to throw off your km # right?
        }
        strip.show(); //ThomasB - based on the documentation, you need to call this whenever you want the updated color to be pushed to the pixels. Otherwise, nothing will happen.
      ...
      {
          //blink all red and off
    for (int ii; ii < StripSize; ii++){
  ...
  if (toggle == true && start != 1)
  {
    //blink all green and off
    for (int ii; ii < StripSize; ii++){
